#include "src/kernels/decoderattn.h"
...
__device__ T blockReduceSum(T val){
    ...
    int lane_id=tid%32;
    static __shared__ T warpss[64];
    val=warpReduceSum(val);
    if(lane_id==0) warpss[wid]=val;
    __syncthreads();
    T out=tid<warp_num?warpss[tid]:0;
...
__device__ T blockReduceMax(T val){
    ...
    __syncthreads();
    T out=tid<warp_num?warpss[tid]:0;
    return warpReduceMax(out);
}
//SM cuda core 寄存器 共享内存 cache1 cache2 显存  
//计算、访存、计算与通信，cudastream、cudagraph

//cpu avx  流水式并行。
//    block threads
template<typename T>
__global__ void decoderattn(T* q,T* k,T* v,T* qkv_bias,T* k_cache,T* v_cache,T* output,int batch_size,int head_num,int kv_head_num,
   int max_seq_len,int head_size,int step){  //这是第一个版本，完全没有用其他的东西。
   int tid=threadIdx.x;
   ...
   int cache_offset=q_batch_id*kv_head_num*max_seq_len*head_size+kv_head_id*max_seq_len*head_size+tid;
   float scale = rsqrt(float(head_size));
    extern __shared__ char sqk[];   //batch_size*head_num*1*head_size  batch_size*head_num*max_k_len*head_size
    T* sq_scalar=reinterpret_cast<T*>(sqk);                   
    float* logits=reinterpret_cast<float*>(sq_scalar+head_size);
    if(tid<head_size) sq_scalar[tid]=q[q_offset];
    //这里是将所有的都存在sq_scalar中。
    __syncthreads();
    if(tid==0&&blockIdx.x==1)printf("****%f**\n",sq_scalar[tid]);
    for(int iter=0;iter<step;iter++){
        T vec_k=k_cache[cache_offset+iter*head_size];
        if(iter==step-1&&tid<head_size){
            k_cache[cache_offset+iter*head_size]=k[k_offset];
            vec_k=k[k_offset];
        }
        T q_acc=sq_scalar[tid]*vec_k*scale;//这个东西相当于放在寄存器里面。 由于一个block没有处理好几行，因此不存在复用
        if(blockIdx.x==1)printf("%f*%f**%f**\n",vec_k,sq_scalar[tid],q_acc);
        T attn_score=blockReduceSum<T>(q_acc);
        if(tid==0)
            logits[iter]=attn_score;//这里就得到了每个gemv一列的结果。
        if(blockIdx.x==0&&tid==0) printf("###和为：%f\n",logits[iter]); 
        __syncthreads();
    }
    T local_logits=tid<step?(T)logits[tid]:0;
    __shared__ float row_max,fenmu;
    T block_max=blockReduceMax<T>(local_logits);
    if(tid==0) row_max=block_max;
    if(blockIdx.x==1&&tid==0) printf("###最大值为：%f\n",row_max); 
    __syncthreads();
    printf("step为：%d\n",step);
    T fenzi=tid<step?expf(logits[tid]-row_max):0;
    if(blockIdx.x==1&&tid<step) printf("分子为：%lf,logits为：%f,row_max：%f,tid：%d\n",expf(logits[tid]-row_max),logits[tid],row_max,tid); 
    T block_fenmu=blockReduceSum<T>(fenzi);
    if(tid==0) fenmu=block_fenmu+1e-6;
    if(blockIdx.x==1&&tid==0) printf("###分母为：%f\n",fenmu); 
    __syncthreads();
    if(tid<step) logits[tid]=(T)fenzi/fenmu;
    __syncthreads();   //只要一旦涉及到线程就需要同步。

    if(tid<head_size){
        T o=(T)0;
        for(int iter=0;iter<step;iter++){
            T v_now=v_cache[cache_offset+iter*head_size];
            if(iter==step-1) {v_cache[cache_offset+iter*head_size]=v[k_offset];
                v_now=v[k_offset];
            }
            o+=v_now*logits[iter];
            if(blockIdx.x==1) printf("%f##%f###：%f\n",v_now,logits[iter],o); 
...
template<typename T>
void launchdecoderattn(TensorWrapper<T> *qkv_buf,BaseWeight<T>& qkv,TensorWrapper<int>* layer_id,TensorWrapper<T>* k_cache,TensorWrapper<T> *v_cache,
                       TensorWrapper<bool>* finished,TensorWrapper<int>* step,TensorWrapper<T>* mha_output){
   int batch_size=qkv_buf->shape[0];
   int qkv_head_num=qkv_buf->shape[1];
   printf("qkv_head_num:%d\n",qkv_head_num);
   int head_size=qkv_buf->shape[2];

   int kv_head_num=k_cache->shape[2];
   int max_seq_len=k_cache->shape[3];

   int head_num=qkv_head_num-2*kv_head_num;
   int cur_step=step->getVal();
   int layer=layer_id->getVal();
   size_t smem_size_bytes=head_size*sizeof(T)+cur_step*sizeof(float);//这里表明我们只取部分共享内存。对于q、k、v来说，我们只取q
   int layer_offset=batch_size*kv_head_num*max_seq_len*head_size*layer;  //这里表明取了若干的layer
    
   T* qkv_data=qkv_buf->data;
   T* q=qkv_data;
   T* k=qkv_data+head_num*head_size;
   T* v=qkv_data+head_num*head_size+kv_head_num*head_size;
   
  // rotary_embedding_base=static_params.rotary_embedding_base;
   //rotary_embedding_dim=static_params.rotary_embedding_dim;  已经将rope给单独提出来了，所以这个部分是没有rope的

   dim3 grid(batch_size*head_num);
   dim3 block(head_size);

   decoderattn<T><<<grid,block,smem_size_bytes>>>(q,k,v,qkv.bias,k_cache->data+layer_offset,v_cache->data+layer_offset,mha_output->data,batch_size,head_num,kv_head_num,
       max_seq_len,head_size,cur_step);
   
}

template void launchdecoderattn(TensorWrapper<float> *qkv_buf,BaseWeight<float>& qkv,TensorWrapper<int>* layer_id,TensorWrapper<float>* k_cache,TensorWrapper<float> *v_cache,
...
// template<typename T>
// void launchdecoderattn(TensorWrapper<T>* qkv_buf,
//                             BaseWeight<T>& qkv,
...
// template void launchdecoderattn(TensorWrapper<float>* qkv_buf,
...
// template void launchdecoderattn(TensorWrapper<half>* qkv_buf,
//                             BaseWeight<half>& qkv,
//                             TensorWrapper<int>* layer_id,
