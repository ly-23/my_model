...
int main()
{
    ...
    launchConcatKV(in_ksrc, in_vsrc, out_kdst, out_vdst, cur_q_len, history_len, layer_id);
    ...
    for (int i = batch_size * (1) * kv_head_num * head_size; i < batch_size * 2* kv_head_num * head_size; i++)
    {
        printf("index = %d\n", i);
        printf("res k = %f\n", h_k_dst[i]);
