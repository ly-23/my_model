            ...
            for (int head = 0; head < head_num; head++) {
                ...
                for (int d = 0; d < head_size; d++) {
                    ...
                    q[b * qbatchstride + s * head_num * head_size + head * head_size + d] = 
                            QKV[b * qbatchstride + s * head_size + head * head_size + d];
                }
                   //RoPE
                for (int d = 0; d < head_size / 2; d++) {
                    float x0 = q[b * qbatchstride + s  * head_size + head * head_size*seq_len + d];
                    float x1 = q[b * qbatchstride + s  * head_size + head * head_size*seq_len + d + 64];
                    ...
                    q[b * qbatchstride + s * head_size + head * head_size*seq_len + d] = 
                                                                        x0 * cos(inv_freq) - x1 * sin(inv_freq);
                    
                    q[b * qbatchstride + s  * head_size + head * head_size*seq_len  + d + 64] = 
                                                                        x1 * cos(inv_freq) + x0 * sin(inv_freq);
                    
                                                                // if(b==0&&s==0&&head<3)
                                                                //         std::cout<<x0 <<"  "<<cos(inv_freq)<<"  "<< x1 <<"  "<< sin(inv_freq)<<" "<<q[b * qbatchstride + s * head_num * head_size + head * head_size + d]<<"  "<<q[b * qbatchstride + s * head_num * head_size + head * head_size + d + 64]<<std::endl;
...
//    Tensor qkv_bias_buf(Device::GPU, type, {(head_num + 2 * kv_head_num), head_size}, dqkv_bias);
    ...
    CPUfunc(hq,
            ...
            rotary_embedding_base);   //这个函数部分就不对
    ...
    for(int i=0;i<300;i++){
        std::cout<<q[i]<<"  "<<hq[i]<<"  "<<k[i]<<"  "<<hk[i]<<std::endl;
    }
